...
/**
 * Legacy Neo4jSafe options interface for backward compatibility
...
/**
 * Legacy Neo4jSafe decorator for backward compatibility
 * @deprecated Use @Safe() instead
 */
export function Neo4jSafe(options?: Neo4jSafeOptions): MethodDecorator {
  // Convert legacy options to new SafeConfig format
  const safeConfig: SafeConfig = {
    strict: options?.validateParams ?? true,
    log: options?.logTransformations ?? false,
    transforms: {
      autoSerialize: options?.autoSerialize ?? true,
      autoInt: options?.autoInt ?? true,
    ...
    rules: {
      ...
      maxDepth: 10,
      sanitizeHtml: false, // Legacy behavior
    },
  };
