 ...
 */
export interface MultiTenantQueryResult<T extends Neo4jRecordShape = Neo4jRecordShape> extends QueryResult {
  /** Tenant metadata */
  tenantMetadata?: {
    tenantId: string;
    databaseName: string;
...
export class TenantConnectionManager implements OnModuleDestroy {
  ...
  private async createTenantConnection(tenantDatabase: string): Promise<void> {
    ...
    const tenantConfig = {
      ...
      maxConnectionPoolSize: Math.max(10, Math.floor(this.baseConfig.maxConnectionPoolSize / 4)),
      connectionAcquisitionTimeout: this.baseConfig.connectionAcquisitionTimeout || 60000,
    ...
    console.log(`Created connection pool for tenant database: ${tenantDatabase}`);
  }

  /**
   * Ensure tenant database exists in Neo4j
   */
  private async ensureTenantDatabase(driver: Driver, databaseName: string): Promise<void> {
   ...
   */
  private async waitForDatabaseAvailable(driver: Driver, databaseName: string, maxAttempts = 30): Promise<void> {
  ...
  getConnectionStats(): Record<string, any> {
    ...
    for (const [tenantDatabase, driver] of this.connectionPools) {
  ...
  async onModuleDestroy(): Promise<void> {
    const closePromises = Array.from(this.connectionPools.values()).map(driver => driver.close());
...
export class MultiTenantNeo4jService {
  constructor(
    ...
    @Inject('BASE_NEO4J_SERVICE') private readonly baseNeo4jService: Neo4jService
  ) {}
  ...
  async run<T extends Neo4jRecordShape = Neo4jRecordShape>(
    ...
    // Get tenant database
    const tenantDatabase = options?.tenantDatabase || await this.tenantContext.getTenantDatabase();
  ...
  async getTenantStats(): Promise<{
    ...
    const result = await this.read(async (session) => {
      ...
      return {
        ...
        relationshipTypes: typesResult.records.map(r => r.get('relationshipType')),
   ...
   */
  private async validateTenantLimits(cypher: string, params?: Record<string, unknown>): Promise<void> {
    ...
    if (limits.maxRelationships && stats.relationshipCount >= limits.maxRelationships) {
      if (cypher.toLowerCase().includes('create') && cypher.toLowerCase().includes('relationship')) {
  ...
  private async trackQueryAnalytics(
    ...
    console.log(`Analytics: Tenant ${tenantId} executed query in ${executionTime}ms, returned ${result.records.length} records`);
  }
