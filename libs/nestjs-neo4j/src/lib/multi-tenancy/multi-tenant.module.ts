...
} from './multi-tenant-neo4j.service';
...
export class MultiTenantNeo4jModule {
  ...
  private static createAsyncProviders(
    options: MultiTenantModuleAsyncOptions
  ): Provider[] {
    return [
      {
        provide: 'MULTI_TENANT_OPTIONS',
      ...
      {
        provide: 'TENANT_RESOLUTION_STRATEGY',
        useFactory: (config: MultiTenantModuleOptions) =>
      ...
      },
      {
        provide: 'TENANT_CONFIG_PROVIDER',
        useFactory: (config: MultiTenantModuleOptions) =>
          this.createConfigProvider(config.configProvider),
        inject: ['MULTI_TENANT_OPTIONS'],
      },
    ];
  }
  ...
  private static createConfigProvider(
    provider?: 'memory' | 'database' | TenantConfigProvider
  ): TenantConfigProvider {
...
export class MultiTenantConfigurations {
   ...
   */
  static headerBased(headerName = 'x-tenant-id'): MultiTenantModuleOptions {
    return {
   ...
   */
  static jwtBased(userProperty = 'tenantId'): MultiTenantModuleOptions {
    return {
   ...
   */
  static pathBased(paramName = 'tenantId'): MultiTenantModuleOptions {
    return {
