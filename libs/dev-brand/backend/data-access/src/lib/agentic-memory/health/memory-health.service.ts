  ...
  async getMetrics(): Promise<{
    ...
    try {
      ...
      try {
        ...
        totalDocuments = await this.chromaDBService.countDocuments(
          'memory-entries'
        );
      } catch {
      ...
      try {
        ...
        try {
          const memoryResults = await this.chromaDBService.getDocuments(
            'memory-entries',
            {
          ...
          if (memoryResults.metadatas) {
            ...
            for (const metadata of metadatas as Array<{
              threadId?: string;
            } | null>) {
