...
/**
 * Command type for multi-agent workflow control
 * Based on LangGraph Command pattern
 */
export interface Command<TState extends AgentState = AgentState> {
  /**
   * Command type (goto, retry, skip, stop, update, end, error)
   */
  ...
  /**
   * Maximum retry attempts (for retry commands)
   */
  maxAttempts?: number;

  /**
   * Error message (for error commands)
   */
  error?: string;
...
/**
 * Service for processing multi-agent workflow commands
 * Implements the LangGraph Command pattern for agent coordination
 */
@Injectable()
export class CommandProcessorService {
  ...
  async processCommand<
    TState extends CommandProcessingState = CommandProcessingState
  >(
   ...
   */
  private async buildStateUpdates<TState extends CommandProcessingState>(
    command: Command<TState>,
    currentState: TState,
    sourceNodeId: string
  ): Promise<Partial<TState>> {
   ...
   * Handle goto command
   */
  private handleGotoCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
   ...
   */
  private handleRetryCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
    baseUpdates: Partial<TState>,
    currentState: TState
  ): Partial<TState> {
   ...
   * Handle skip command
   */
  private handleSkipCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
    baseUpdates: Partial<TState>,
    currentState: TState
  ): Partial<TState> {
   ...
   * Handle stop command
   */
  private handleStopCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
    baseUpdates: Partial<TState>,
    currentState: TState
  ): Partial<TState> {
   ...
   */
  private handleUpdateCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
    baseUpdates: Partial<TState>,
    currentState: TState
  ): Partial<TState> {
   ...
   * Handle end command
   */
  private handleEndCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
    baseUpdates: Partial<TState>,
    currentState: TState
  ): Partial<TState> {
   ...
   */
  private handleErrorCommand<TState extends CommandProcessingState>(
    command: Command<TState>,
    baseUpdates: Partial<TState>,
    currentState: TState
  ): Partial<TState> {
   ...
   */
  private applyCommandMetadata<TState extends CommandProcessingState>(
   ...
   */
  private createErrorState<TState extends CommandProcessingState>(
    error: Error,
    sourceNodeId: string,
    currentState: TState
  ): Partial<TState> {
   ...
   * Validate a command
   */
  validateCommand<TState extends CommandProcessingState>(
    command: Command<TState>
   ...
   */
  isCommand<TState extends CommandProcessingState>(
    value: unknown
