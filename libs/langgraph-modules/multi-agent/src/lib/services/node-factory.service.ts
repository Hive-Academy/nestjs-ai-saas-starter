...
export class NodeFactoryService {
  ...
  async createSupervisorNode(
  ...
  ): Promise<(state: AgentState, runConfig?: RunnableConfig) => Promise<Partial<AgentState>>> {
    ...
    return async (state: AgentState): Promise<Partial<AgentState>> => {
      try {
        ...
        // Fallback routing
        this.logger.warn('Supervisor failed to make routing decision, ending workflow');
        return {
          messages: [response],
        ...
        return {
          messages: [new AIMessage('Supervisor encountered an error. Ending workflow.')],
          next: MULTI_AGENT_CONSTANTS.END,
          metadata: {
            ...state.metadata,
            supervisorError: error instanceof Error ? error.message : 'Unknown error',
          },
        };
      }
    };
  ...
  private createRoutingTool(config: SupervisorConfig) {
    return {
      name: config.routingTool?.name || MULTI_AGENT_CONSTANTS.DEFAULT_ROUTING_TOOL,
      description: config.routingTool?.description || 'Select the next worker to act.',
      schema: {
        type: 'object',
        properties: {
          next: {
      ...
      for (const message of result.messages) {
        if (message._getType() === 'ai' && 'tool_calls' in message) {
          ...
          for (const toolCall of toolCalls) {
            const matchingTool = handoffTools.find(tool => 
   ...
   */
  createNodeWrapper<T extends (state: AgentState, config?: RunnableConfig) => Promise<Partial<AgentState>>>(
    nodeFunction: T,
    agentId: string,
    ...
    const wrappedFunction = async (state: AgentState, config?: RunnableConfig) => {
      ...
      try {
        ...
        this.logger.debug(`Node ${agentId} (${nodeType}) executed successfully`, {
          executionTime,
