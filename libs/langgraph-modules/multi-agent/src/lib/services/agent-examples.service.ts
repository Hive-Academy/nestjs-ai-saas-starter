...
export class AgentExamplesService {
  ...
  createResearchAssistantNetwork(): AgentNetwork {
    const researcherAgent: AgentDefinition = {
      id: 'researcher',
      name: 'Researcher',
      description: 'Conducts thorough research and gathers information on topics',
      systemPrompt: 'You are a research specialist. Gather comprehensive information and provide detailed analysis.',
...
Research complete. Passing to writer for documentation.`;
    ...
    const writerAgent: AgentDefinition = {
      id: 'writer',
      name: 'Writer',
      description: 'Creates well-structured documents and reports from research',
      systemPrompt: 'You are a technical writer. Create clear, well-structured documentation.',
      nodeFunction: async (state: AgentState) => {
  ...
  createCustomerSupportSwarm(): AgentNetwork {
    const triageAgent: AgentDefinition = {
      ...
      nodeFunction: async (state: AgentState) => {
        ...
        if (inquiry.toLowerCase().includes('billing') || inquiry.toLowerCase().includes('payment')) {
          return {
            messages: [new AIMessage('I see this is billing-related. Transferring to our billing specialist.')],
            next: 'billing',
            task: 'Handle billing inquiry',
          };
        } else if (inquiry.toLowerCase().includes('technical') || inquiry.toLowerCase().includes('error')) {
          return {
            messages: [new AIMessage('This appears technical. Connecting you to technical support.')],
            next: 'technical', 
