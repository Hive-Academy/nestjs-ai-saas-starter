...
export class AlertingService implements IAlertingService, OnModuleDestroy {
  ...
  private readonly notificationProviders = new Map<string, INotificationProvider>();
  ...
  async onModuleDestroy(): Promise<void> {
    ...
    this.logger.log(`AlertingService shutdown - Evaluations: ${this.evaluationCount}, Alerts: ${this.alertCount}`);
  }
  ...
  async createRule(rule: AlertRule): Promise<string> {
    try {
      ...
      throw new AlertingError(
        `Failed to create alert rule: ${rule.name}`,
  ...
  async updateRule(ruleId: string, updates: Partial<AlertRule>): Promise<void> {
    ...
    try {
      ...
      throw new AlertingError(
        `Failed to update alert rule: ${ruleId}`,
  ...
  async triggerAlert(alert: Alert): Promise<void> {
    try {
      ...
      throw new AlertingError(
        `Failed to trigger alert: ${alert.name}`,
  ...
  registerNotificationProvider(type: string, provider: INotificationProvider): void {
   ...
   * Send notifications to all configured channels
   */
  private async sendNotifications(alert: Alert, channels: readonly NotificationChannel[]): Promise<void> {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'rejected') {
        this.logger.error(`Notification failed for channel ${channel.name}:`, result.reason);
      } else {
   ...
   */
  private async sendNotificationSafely(alert: Alert, channel: NotificationChannel): Promise<void> {
    if (!channel.enabled) {
      return;
    }
    ...
    if (!provider) {
      this.logger.warn(`No notification provider found for type: ${channel.type}`);
      return;
    }
    ...
    try {
      ...
      this.logger.error(`Notification provider error for ${channel.type}:`, error);
      throw error;
    }
  }
  ...
  private validateRule(rule: AlertRule): void {
    ...
    if (!rule.condition.metric || !rule.condition.operator || rule.condition.threshold === undefined) {
      throw new Error('Rule condition must have metric, operator, and threshold');
    }
   ...
   */
  private async formatAlertMessage(rule: AlertRule, context?: Record<string, unknown>): Promise<string> {
    const condition = rule.condition;
   ...
   */
  private async gatherAlertContext(rule: AlertRule): Promise<Record<string, unknown>> {
    return {
      ruleId: rule.id,
      ruleName: rule.name,
   ...
   */
  async evaluateRuleWithData(rule: AlertRule, data: Record<string, unknown>): Promise<boolean> {
   ...
   * Clean up old alerts
   */
  async cleanupOldAlerts(maxAge: number = 24 * 60 * 60 * 1000): Promise<number> {
    ...
    const totalRemoved = initialCount - this.alertHistory.length + removedActiveCount;
   ...
   */
  async exportAlerts(format: 'json' | 'csv' = 'json', filter?: { ruleId?: string; severity?: string }): Promise<string> {
    ...
    if (format === 'json') {
      ...
      const headers = ['id', 'ruleId', 'name', 'severity', 'message', 'timestamp', 'acknowledged'];
