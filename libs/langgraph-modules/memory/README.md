...
```bash
npm install @langgraph-modules/memory
```
...
import { LanggraphModulesMemoryModule } from '@langgraph-modules/memory';
...
import { MemoryFacadeService, SemanticSearchService } from '@langgraph-modules/memory';
...
export class ConversationService {
  ...
  async storeMessage(threadId: string, content: string, userId?: string) {
    return this.memoryFacade.store(threadId, content, {
      type: 'conversation',
      source: 'user',
      importance: 0.7,
    }, userId);
...
```typescript
LanggraphModulesMemoryModule.forRootAsync({
...
const context = await memoryFacade.searchForContext(
...
const summary = await memoryFacade.summarize(
  'thread-123',
  messages,
  {
...
const contextResults = await semanticSearch.contextAwareSearch(
...
const similar = await semanticSearch.findSimilarMemories(
  'memory-id-123',
  5,
  0.7
);
...
const summary = await summarizationService.createSummary(
  messages,
  {
...
import { MemoryHealthService } from '@langgraph-modules/memory';
