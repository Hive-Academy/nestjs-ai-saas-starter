  ...
  implements MemoryServiceInterface, OnModuleInit
{
  ...
  constructor(
    ...
    this.collectionName = this.configService.get<string>('memory.collection', 'memory-entries');
  }
  ...
  public async storeBatch(
    ...
    try {
      const memories: MemoryEntry[] = entries.map((e) => this.buildMemoryEntry(threadId, e.content, e.metadata));
  ...
  const memories = this.convertToMemoryEntries(results as GetResult<ChromaMeta>);
  ...
  await this.chromaDB.deleteDocuments(this.collectionName, [...memoryIds]);
  ...
  const ids = Array.isArray(existing.ids?.[0]) ? existing.ids?.[0] ?? [] : (existing.ids ?? []);
   ...
   */
  public async getConversationFlow(threadId: string): Promise<Array<{ memoryId: string; content: string; type: string; createdAt: Date; connections: string[] }>> {
    try {
  ...
  const msg = error instanceof Error ? error.message : String(error);
  this.logger.error(`Auto-summarization failed for thread ${threadId}: ${msg}`);
    }
  }
  ...
  private async upsertGraphForMemory(memory: MemoryEntry, userId?: string): Promise<void> {
  await this.neo4j.write(async (session: Session) => {
      await session.run(
      ...
      await session.run(
        ...
        { threadId: memory.threadId, memoryId: memory.id, createdAt: memory.createdAt.toISOString() }
      );
  ...
  private async upsertGraphForMemoriesBatch(memories: MemoryEntry[], userId?: string): Promise<void> {
  if (!memories.length) { return; }
  await this.neo4j.write(async (session: Session) => {
      for (const memory of memories) {
        await session.run(
  ...
  private convertToMemoryEntries(results: GetResult<ChromaMeta>): MemoryEntry[] {
  ...
  const md: Record<string, unknown> = (metadatas[i] ?? {}) as Record<string, unknown>;
      const additional = md.additionalMetadata ? safeParseJson(String(md.additionalMetadata)) : {};
      memories.push({
        id: ids[i],
        threadId: String(md.threadId ?? ''),
        content: documents[i] ?? '',
        embedding: (embeddings[i] ?? undefined) as unknown as number[] | undefined,
        metadata: {
        ...
        lastAccessedAt: md.lastAccessedAt ? new Date(String(md.lastAccessedAt)) : undefined,
        accessCount: (md.accessCount as number) || 0,
        relevanceScore: distances[i] !== undefined && distances[i] !== null ? 1 - Number(distances[i]) : undefined,
      });
    }
    return memories;
  }
  ...
  private async createTypeSpecificRelationships(session: Session, memory: MemoryEntry): Promise<void> {
