...
export class MemoryCoreService implements MemoryServiceInterface, OnModuleInit, OnModuleDestroy {
  ...
  private async initializeNeo4jSchema(): Promise<void> {
    try {
      ...
      this.logger.warn('Failed to initialize Neo4j schema (may already exist)', error);
    ...
    try {
      ...
      if (this.isSemanticSearchEnabled()) {
        try {
          ...
          this.logger.warn('Failed to generate embedding, storing without semantic search capability', error);
      ...
      await this.executeChromaDBOperation(
        async () => this.chromaDBService.addDocuments(this.collectionName, [{
          id: entry.id,
          document: content,
          metadata: {
      ...
      throw MemoryStorageError.documentStorage('store', threadId, error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async storeBatch(
    ...
    try {
      ...
      throw MemoryStorageError.documentStorage('storeBatch', threadId, error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async retrieve(threadId: string, limit?: number): Promise<readonly MemoryEntry[]> {
    try {
      const results = await this.executeChromaDBOperation(
        async () => this.chromaDBService.getDocuments(this.collectionName, {
          where: { threadId },
  ...
  const memories = this.convertFromGetResults(results);
      ...
      throw MemoryStorageError.documentRetrieval('retrieve', threadId, error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  let results: MemoryEntry[];
      ...
      if (options.query && this.isSemanticSearchEnabled()) {
        try {
          ...
          const searchResults: ChromaSearchResult = await this.executeChromaDBOperation(
            async () => this.chromaDBService.searchDocuments(this.collectionName, undefined, [queryEmbedding], {
            ...
            async () => {
              this.logger.warn('Semantic search failed, falling back to regular filtering');
          ...
          this.logger.warn('Semantic search failed completely, falling back to regular filtering', error);
      ...
      throw MemoryStorageError.searchOperation('search', query, error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async delete(threadId: string, memoryIds?: readonly string[]): Promise<number> {
    try {
      ...
      throw MemoryStorageError.documentStorage('delete', threadId, error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  public async getStats(): Promise<MemoryStats> {
    try {
      ...
      throw MemoryStorageError.documentRetrieval('getStats', 'all', error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  private async storeMemoryInNeo4j(entry: MemoryEntry): Promise<void> {
    try {
      ...
      const neo4jError = MemoryRelationshipError.graphStorage('storeMemoryInNeo4j', entry.threadId, entry.id, error instanceof Error ? error : new Error(String(error)));
      this.logger.error(`Failed to store memory ${entry.id} in Neo4j`, neo4jError.toJSON());
   ...
   */
  private async createTypeSpecificRelationships(session: any, entry: MemoryEntry): Promise<void> {
  ...
  private async storeMemoriesInNeo4j(entries: MemoryEntry[]): Promise<void> {
    try {
      ...
      const neo4jError = MemoryRelationshipError.graphStorage('storeMemoriesInNeo4j', entries[0]?.threadId ?? 'unknown', 'batch', error instanceof Error ? error : new Error(String(error)));
      this.logger.error('Failed to store batch memories in Neo4j', neo4jError.toJSON());
      // Don't throw - Neo4j failures shouldn't prevent ChromaDB storage
    }
  }
  ...
  private async deleteMemoriesFromNeo4j(memoryIds: string[]): Promise<void> {
    try {
      ...
      const neo4jError = MemoryRelationshipError.relationshipQuery('deleteMemoriesFromNeo4j', 'unknown', error instanceof Error ? error : new Error(String(error)));
      this.logger.error('Failed to delete memories from Neo4j', neo4jError.toJSON());
  ...
  private async withTimeout<T>(
    ...
    return new Promise<T>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(MemoryTimeoutError.operationTimeout(operationName, service, timeoutMs, threadId));
      }, timeoutMs);
  ...
  private async executeChromaDBOperation<T>(
    ...
    const timeout = this.configService.get<number>('memory.timeout.chromadb', 30000);

    try {
      return await this.withTimeout(operation, timeout, operationName, 'chromadb', threadId);
    } catch (error) {
  ...
  private async executeNeo4jOperation<T>(
    ...
    const timeout = this.configService.get<number>('memory.timeout.neo4j', 15000);

    try {
      return await this.withTimeout(operation, timeout, operationName, 'neo4j', threadId);
    } catch (error) {
      const neo4jError = wrapMemoryError(error, operationName, 'neo4j', { threadId });
      this.logger.error(`Neo4j operation ${operationName} failed, continuing without graph features`, neo4jError.toJSON());
  ...
  private async executeEmbeddingOperation<T>(
    ...
    const timeout = this.configService.get<number>('memory.timeout.embedding', 20000);
    let lastError: Error;

    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        return await this.withTimeout(operation, timeout, operationName, 'embedding');
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(String(error));

        if (attempt < retries) {
          const delay = Math.min(1000 * Math.pow(2, attempt - 1), 5000); // Exponential backoff, max 5s
          this.logger.warn(`Embedding operation ${operationName} failed (attempt ${attempt}/${retries}), retrying in ${delay}ms`, lastError);
          await new Promise(resolve => setTimeout(resolve, delay));
        }
      }
    }

    throw MemoryEmbeddingError.embeddingGeneration(operationName, 1, lastError!);
  }
