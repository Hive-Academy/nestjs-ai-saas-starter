...
import { MemoryEntry, MemorySearchOptions, UserMemoryPatterns } from '../interfaces/memory.interface';
...
export class SemanticSearchService {
   ...
   */
  public async hybridSearch(options: MemorySearchOptions & {
  ...
  const vectorResults = await this.vectorSearch(options);
    ...
    for (const memory of vectorResults.slice(0, 5)) { // Limit to top 5 for performance
      ...
      const boostFactor = options.boostRelated ?? 0.3;
          relatedMemoriesMap.set(rel.memoryId, Math.max(existingScore, rel.similarity * boostFactor));
        }
      }
  ...
  const relatedMemories = await this.getMemoriesByIds(relatedIds);
        ...
        for (const memory of relatedMemories) {
          ...
          memory.relevanceScore = (memory.relevanceScore ?? 0) + relationshipScore;
        }

        // Merge and deduplicate results
        const allResults = this.mergeAndDeduplicateResults(vectorResults, relatedMemories);
  ...
  public async searchForAnswer(
    ...
    try {
      ...
      const allMemories = this.mergeAndDeduplicateResults(directResults, factualResults);
  ...
  public async createSemanticLinks(batchSize = 50): Promise<void> {
    try {
      ...
      while (true) {
        ...
        this.logger.debug(`Processed ${processedCount} memories for semantic linking`);
      }

      this.logger.log(`Completed semantic link creation for ${processedCount} memories`);
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  const scores = memories.map((m) => m.relevanceScore ?? 0);
    ...
    const factualBoost = memories.filter(m =>
      m.metadata.type === 'fact' || m.metadata.type === 'context'
    ).length * 0.1;

    // Boost confidence for persistent memories (important information)
    const persistentBoost = memories.filter(m => m.metadata.persistent).length * 0.05;
   ...
   */
  private async vectorSearch(options: MemorySearchOptions): Promise<MemoryEntry[]> {
    const where: Where | undefined = this.buildWhereFilter(options);

    let memories: MemoryEntry[];
    if (options.query) {
      const searchResults: ChromaSearchResult = await this.chromaDBService.searchDocuments(
        'memory-entries',
        [options.query],
        undefined,
        {
  ...
  memories = this.convertToMemoryEntries(getResults as unknown as GetResult<Record<string, string | number | boolean | null>>);
    }
  ...
  private postFilterAndSort(memories: MemoryEntry[], options: MemorySearchOptions): MemoryEntry[] {
    let result = [...memories];
  ...
  return this.convertToMemoryEntries(results as unknown as GetResult<Record<string, string | number | boolean | null>>);
    } catch (error) {
  const msg = error instanceof Error ? error.message : String(error);
   ...
   */
  private async getAllMemoriesBatch(offset: number, limit: number): Promise<MemoryEntry[]> {
    try {
  const results = await this.chromaDBService.getDocuments(
        'memory-entries',
        {
  ...
  private async findRelatedMemories(
    ...
    maxDepth = 2,
    limit = 10
  ): Promise<Array<{ memoryId: string; relationshipPath: string[]; similarity: number }>> {
   ...
   */
  public async getConversationFlow(threadId: string): Promise<Array<{
    memoryId: string; content: string; type: string; createdAt: Date; connections: string[];
  }>> {
  const result = await this.neo4jService.run<{
      memoryId: string;
      content: string;
   ...
   */
  public async getUserMemoryPatterns(userId: string): Promise<UserMemoryPatterns> {
    const { topics, styles, stats } = await this.neo4jService.runInReadTransaction(async (tx) => ({
  ...
  private async fetchUserTopics(tx: Transaction, userId: string): Promise<Array<{ tag: string; frequency: number; avgImportance: number }>> {
    const qb = cypher()
      .match('(u:User {id: $userId})-[:OWNS_THREAD]->(t:Thread)-[:HAS_MEMORY]->(m:Memory)')
  ...
  private async fetchUserStyles(tx: Transaction, userId: string): Promise<Array<{ content: string; importance: number }>> {
    const qb = cypher()
      .match('(u:User {id: $userId})-[:OWNS_THREAD]->(t:Thread)-[:HAS_MEMORY]->(m:Memory)')
  ...
  private async fetchUserStats(tx: Transaction, userId: string): Promise<{ totalThreads: number; totalMemories: number; avgImportance: number; lastMemoryAt: string | number | Date }> {
    const qb = cypher()
      .match('(u:User {id: $userId})-[:OWNS_THREAD]->(t:Thread)-[:HAS_MEMORY]->(m:Memory)')
      .return('count(DISTINCT t) as totalThreads, count(m) as totalMemories, avg(m.importance) as avgImportance, max(m.createdAt) as lastMemoryAt')
      .addParameter('userId', userId);
    ...
    return rec
      ...
      : { totalThreads: 0, totalMemories: 0, avgImportance: 0, lastMemoryAt: Date.now() };
  }
  ...
  /* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, complexity, no-nested-ternary */
  private convertToMemoryEntries(results: ChromaSearchResult | GetResult<Record<string, string | number | boolean | null>>): MemoryEntry[] {
    ...
    const ids = Array.isArray((results as any).ids[0]) ? (results as any).ids[0] : (results as any).ids;
  const documents = (results as any).documents ? (Array.isArray((results as any).documents[0]) ? (results as any).documents[0] : (results as any).documents) : [];
  const metadatas = (results as any).metadatas ? (Array.isArray((results as any).metadatas[0]) ? (results as any).metadatas[0] : (results as any).metadatas) : [];
  const embeddings = (results as any).embeddings ? (Array.isArray((results as any).embeddings[0]) ? (results as any).embeddings[0] : (results as any).embeddings) : [];
  const distances = (results as any).distances ? (Array.isArray((results as any).distances[0]) ? (results as any).distances[0] : (results as any).distances) : [];

    for (let i = 0; i < ids.length; i++) {
  const md = metadatas[i] ?? {};
      const additional = md.additionalMetadata ? safeParseJson(md.additionalMetadata as string) : {};
  ...
  embedding: embeddings[i] ?? undefined,
        metadata: {
          ...
          tags: md.tags ? String(md.tags).split(',').filter(Boolean) : undefined,
        ...
        lastAccessedAt: md.lastAccessedAt ? new Date(md.lastAccessedAt as string) : undefined,
        accessCount: (md.accessCount as number) || 0,
  relevanceScore: (distances[i] !== null && distances[i] !== undefined) ? 1 - Number(distances[i]) : undefined,
      });
    }

    return memories;
  }
  /* eslint-enable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, complexity */
}
...
function toNumberSafe(value: unknown): number {
  ...
  if (maybeInt && typeof maybeInt.toNumber === 'function') {
    try { return maybeInt.toNumber(); } catch { /* ignore */ }
  }
