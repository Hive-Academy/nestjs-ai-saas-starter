...
export class ApprovalProcessingService {
  ...
  async processApproval(request: {
    ...
    feedback?: string;
    timestamp: Date;
  }, approvalRequests: Map<string, HumanApprovalRequest>): Promise<{
    success: boolean;
    ...
    try {
      ...
      const approvalRequest = Array.from(approvalRequests.values()).find(
        req => req.executionId === request.executionId && req.nodeId === request.nodeId
      );
      ...
      const result = await this.processApprovalResponse(approvalRequest.id, {
