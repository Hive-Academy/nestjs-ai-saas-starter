...
export class TokenProcessingService {
  ...
  async *streamTokens(
    ...
    try {
      ...
      this.logger.error(`Workflow token streaming failed for ${executionId}:${nodeId}:`, error);
  ...
  async *streamMessageTokens(
    ...
    this.logger.debug(`Streaming tokens from ${messages.length} messages for ${executionId}:${nodeId}`);
   ...
   */
  private tokenizeContent(content: string, config?: StreamTokenDecoratorMetadata): string[] {
  ...
  getBufferStatus(): {
    ...
    if (this.streamingService && 'getActiveTokenStreams' in this.streamingService) {
      ...
      return {
        ...
        totalTokensBuffered: streams.reduce((sum: number, stream: any) => sum + stream.bufferSize, 0),
  ...
  async forceFlushAllBuffers(): Promise<number> {
    if (this.streamingService && 'forceFlushAllBuffers' in this.streamingService) {
