...
import { isMultiAgentDefinition } from '../interfaces/multi-agent-bridge.interface';
import type {
  WorkflowNode,
  WorkflowEdge,
  WorkflowState,
...
export class MultiAgentTranslationService {
   ...
   */
  async translateMultiAgentDefinition<
   ...
   */
  private async translateMultiAgentWorkflow<
    TState extends WorkflowState = WorkflowState
  >(
    definition: MultiAgentWorkflowDefinition,
    instance: object,
  ...
  private convertAgentToNode<TState extends WorkflowState = WorkflowState>(
    ...
    const method =
      (instance as any)[agentDef.agentId] ||
      (instance as any)[`execute${agentId}`];
    ...
    return {
      ...
      description: `Multi-agent: ${
        agentDef.role
      } with capabilities: ${agentDef.capabilities.join(', ')}`,
      handler,
      config: {
          ...
          to: {
            condition: (state: TState) => {
              ...
              return networkState &&
                ...
                networkState.activeAgents.includes(toAgent)
                ? toAgent
                : null;
            },
            routes: {
  ...
  private analyzeNetworkComplexity(
    ...
    const connectionCount = Array.from(
      definition.network.connections.values()
  ...
  private selectNextAgents(
    ...
    const availableAgents = networkState.activeAgents.filter(
      (agentId) =>
        !networkState.completedAgents.includes(agentId) &&
  ...
  private createCommunicationChannels(
    ...
    return {
      ...
      direct: async (targetAgent: string, message: any) => {
        this.logger.debug(
          `Agent ${agentId} sending to ${targetAgent}:`,
          message
        );
      },
  ...
  createWorkflowDefinition<TState extends WorkflowState = WorkflowState>(
    ...
    return {
      ...
      description:
        description ||
        `Multi-agent workflow with ${translationResult.metadata.agentCount} agents using ${translationResult.metadata.coordinationStrategy} strategy`,
  ...
  validateTranslation<TState extends WorkflowState = WorkflowState>(
    ...
    if (
      !hasCoordinator &&
      result.metadata.coordinationStrategy !== 'sequential'
    ) {
      errors.push('Coordinator node missing for non-sequential strategy');
    }

    // Check agent count consistency
    const agentNodes = result.nodes.filter(
      (n) => n.config?.metadata?.type === 'agent'
    );
  ...
  optimizeTranslation<TState extends WorkflowState = WorkflowState>(
    ...
    const optimizedNodes = this.optimizeNodeOrder(
      result.nodes,
