...
import { ToolMetadata, getClassTools, getToolMetadata } from '../decorators/tool.decorator';
...
export class ToolDiscoveryService implements OnModuleInit {
   ...
   */
  async discoverFromProvider(providerClass: any): Promise<ProviderDiscoveryResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      try {
        providerInstance = await this.moduleRef.get(providerClass, { strict: false });
      } catch (error) {
      ...
      for (const toolMeta of toolsMetadata) {
        try {
          ...
          const errorMsg = `Tool ${toolMeta.name} validation failed: ${error instanceof Error ? error.message : String(error)}`;
      ...
      const scanTime = Date.now() - startTime;
      const errorMsg = `Failed to discover from ${providerName}: ${error instanceof Error ? error.message : String(error)}`;
  ...
  async discoverByTag(tags: string[]): Promise<ToolMetadata[]> {
    ...
    this.logger.debug(`Found ${matchingTools.length} tools matching tags: ${tags.join(', ')}`);
   ...
   */
  async discoverForAgent(agentType: AgentType | string): Promise<ToolMetadata[]> {
    ...
    this.logger.debug(`Found ${toolsMetadata.length} tools for agent: ${agentType}`);
  ...
  async analyzeToolDependencies(): Promise<{
    ...
    const composedToolChains: Array<{ parentTool: string; componentTools: string[] }> = [];
    ...
    const analysis = { dependencies, conflicts, orphanedTools, composedToolChains };
  ...
  async refreshProvider(providerClass: any): Promise<void> {
    ...
    try {
      ...
      for (const toolMeta of result.toolsFound) {
        const instance = await this.moduleRef.get(providerClass, { strict: false });
        await this.registerToolWithRegistry(toolMeta, instance);
      }

      this.logger.log(`Refreshed ${result.toolsFound.length} tools from ${providerClass.name}`);
      
    } catch (error) {
      this.logger.error(`Failed to refresh provider ${providerClass.name}:`, error);
      throw error;
    }
  }
  ...
  async validateToolConfiguration(): Promise<{
    ...
    this.logger.debug(`Tool validation complete: ${valid.length} valid, ${invalid.length} invalid`);
  ...
  private async processDiscoveryResults(results: ProviderDiscoveryResult[]): Promise<void> {
    for (const result of results) {
      ...
      for (const toolMeta of result.toolsFound) {
        try {
          const instance = await this.moduleRef.get(toolMeta.className as any, { strict: false });
          await this.registerToolWithRegistry(toolMeta, instance);
        } catch (error) {
      ...
      if (metadata) {
        ...
        if (metadata.tags) {
          for (const tag of metadata.tags) {
            this.discoveryStats.toolsByTag[tag] = (this.discoveryStats.toolsByTag[tag] || 0) + 1;
          }
        }
        
        // Count by version
        const version = metadata.version || 'unknown';
        this.discoveryStats.toolsByVersion[version] = (this.discoveryStats.toolsByVersion[version] || 0) + 1;
  ...
  private async validateToolMetadata(toolMeta: ToolMetadata, instance: any): Promise<void> {
  ...
  private shouldIncludeTool(toolMeta: ToolMetadata): boolean {
    ...
    if (!this.discoveryOptions.discoverComposedTools && toolMeta.tags?.includes('composed')) {
      return false;
    }
  ...
  private async registerToolWithRegistry(metadata: ToolMetadata, instance: any): Promise<void> {
