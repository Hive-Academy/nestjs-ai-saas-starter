...
export const getSupervisorAgentConfig = (): LangGraphModuleOptions => ({
  ...
  defaultLLM: {
    ...
    model: process.env.OPENROUTER_MODEL || process.env.LLM_MODEL || 'openai/gpt-3.5-turbo',
    temperature: parseFloat(process.env.OPENROUTER_TEMPERATURE || process.env.LLM_TEMPERATURE || '0.7'),
    maxTokens: parseInt(process.env.OPENROUTER_MAX_TOKENS || process.env.LLM_MAX_TOKENS || '2048', 10),
  ...
  checkpoint: {
    ...
    storage: (process.env.CHECKPOINT_STORAGE as 'memory' | 'sqlite' | 'redis' | 'postgresql') || 'memory',
    storageConfig: {
      // Redis configuration
  ...
  multiAgent: {
    ...
    agents: (process.env.MULTI_AGENT_LIST || 'supervisor,researcher,analyzer').split(','),
    coordinationStrategy: (process.env.MULTI_AGENT_STRATEGY as 'supervisor' | 'swarm' | 'hierarchical') || 'supervisor',
    maxConcurrentAgents: parseInt(process.env.MULTI_AGENT_MAX_CONCURRENT || '3', 10),
  },
  ...
  monitoring: {
    ...
    alerting: {
      ...
      thresholds: {
        ...
        latency: parseInt(process.env.MONITORING_LATENCY_THRESHOLD_MS || '5000', 10),
      },
    },
  },
  ...
  debug: {
    ...
    logLevel: (process.env.DEBUG_LOG_LEVEL as 'debug' | 'info' | 'warn' | 'error') || 'info',
  },
});
