...
export const KNOWLEDGE_BASE_DATA: KnowledgeDocument[] = [
  DocumentFactory.createKnowledgeDocument({
    ...
    content:
      'Vector databases are specialized databases designed to store and query high-dimensional vectors. They are essential for semantic search, recommendation systems, and AI applications.',
    category: 'database',
  ...
  DocumentFactory.createKnowledgeDocument({
    ...
    content:
      'Embeddings are numerical representations of data that capture semantic meaning. Similarity search uses these embeddings to find related content based on semantic similarity rather than exact keyword matches.',
    category: 'ai',
  ...
  DocumentFactory.createKnowledgeDocument({
    ...
    content:
      'Effective document preprocessing is crucial for optimal vector search performance. This includes text cleaning, chunking strategies, metadata extraction, and embedding generation techniques.',
...
export const PRODUCT_CATALOG_DATA: ProductDocument[] = [
  ...
  DocumentFactory.createProductDocument({
    ...
    content:
      'Advanced fitness tracker with heart rate monitoring, GPS tracking, sleep analysis, and smartphone integration. Water-resistant design for all activities.',
    category: 'electronics',
  ...
  DocumentFactory.createProductDocument({
    ...
    content:
      'Professional ergonomic office chair with lumbar support, adjustable height, and breathable mesh fabric. Designed for long work sessions and optimal comfort.',
    category: 'furniture',
    subcategory: 'office',
    brand: 'ErgoMax',
    price: 299.99,
...
export const SUPPORT_DOCUMENTATION_DATA: SupportDocument[] = [
  DocumentFactory.createSupportDocument({
    ...
    content:
      'To reset your password: 1) Go to the login page, 2) Click "Forgot Password", 3) Enter your email address, 4) Check your email for reset instructions, 5) Follow the link and create a new password.',
    category: 'howto',
    priority: 'medium',
  ...
  DocumentFactory.createSupportDocument({
    ...
    content:
      'If you experience connection problems: 1) Check your internet connection, 2) Clear browser cache and cookies, 3) Disable browser extensions, 4) Try a different browser, 5) Contact support if issues persist.',
  ...
  DocumentFactory.createSupportDocument({
    ...
    content:
      'Our platform complies with GDPR regulations. Users have the right to access, modify, and delete their personal data. We implement encryption, access controls, and regular security audits to protect user information.',
    category: 'policy',
    priority: 'critical',
...
export const RESEARCH_PAPERS_DATA: ResearchDocument[] = [
  DocumentFactory.createResearchDocument({
    ...
    content:
      'This paper presents novel approaches to vector similarity search optimization, focusing on indexing strategies and query performance in high-dimensional embedding spaces.',
  ...
  DocumentFactory.createResearchDocument({
    ...
    content:
      'An empirical study on implementing semantic search systems in enterprise environments, analyzing user satisfaction, search accuracy, and system adoption patterns.',
...
export const MULTI_TENANT_DATA: TenantDocument[] = [
  DocumentFactory.createTenantDocument({
    ...
    content:
      'Internal company policy regarding remote work guidelines, communication protocols, and performance evaluation criteria.',
    visibility: 'private',
    metadata: {
      tenantMetadata: {
  ...
  DocumentFactory.createTenantDocument({
    ...
    content:
      'Quarterly product development roadmap including feature priorities, technical requirements, and resource allocation for the upcoming release cycle.',
    visibility: 'shared',
    metadata: {
      tenantMetadata: {
...
export const TEST_DATA_COLLECTIONS = {
  ...
  getByCategory: (category: string) => {
    return TEST_DATA_COLLECTIONS.allDocuments.filter(
      (doc) => 'category' in doc && doc.category === category
    );
  },
  ...
  getPublicDocuments: () => {
    return TEST_DATA_COLLECTIONS.allDocuments.filter(
      (doc) => !('visibility' in doc) || (doc as any).visibility === 'public'
    );
  },
};
