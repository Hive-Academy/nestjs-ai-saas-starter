...
export class CollectionHelper {
  ...
  async ensureCollection(
    ...
    try {
      ...
      this.logger.logInfo(
        `Collection '${name}' already exists or creation failed`
      );
    }
  }
  ...
  async clearCollection(name: string): Promise<void> {
    try {
      ...
      if (documents.ids && documents.ids[0] && documents.ids[0].length > 0) {
        ...
        this.logger.logInfo(
          `Collection '${name}' cleared (${documents.ids[0].length} documents)`
        );
      } else {
      ...
      this.logger.logWarning(
        `Could not clear collection '${name}': ${
...
export class SearchResultFormatter {
  static formatSearchResults(
    ...
    ids.forEach((id: string, index: number) => {
      ...
      if (document) {
        const preview =
          document.length > 100 ? document.substring(0, 100) + '...' : document;
  ...
  static formatSearchSummary(results: ChromaSearchResult): {
    totalResults: number;
    ...
    if (results.distances?.[0] && results.distances[0].length > 0) {
      const avgDistance =
        results.distances[0].reduce((sum: number, d: number) => sum + d, 0) /
        results.distances[0].length;
...
export class ExampleValidator {
  ...
  static validateDocumentStructure(document: ExampleDocument): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  static validateCollectionConfiguration(config: any): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
...
export class DemoWorkflow {
  ...
  async demonstrateSearch(
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      this.logger.logResults('Search completed', summary, duration);

      const formattedResults = SearchResultFormatter.formatSearchResults(
        result,
        3
      );
