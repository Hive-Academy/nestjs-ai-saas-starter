...
import { ChromaDBModule, ChromaDBFacadeService, ChromaAdminService } from '../../../index';
import { 
  DemoLogger,
  PerformanceTimer,
  ExampleValidator
...
export class CollectionManagementService implements OnModuleInit {
  ...
  private async demonstrateCollectionCreation(): Promise<void> {
    ...
    const { duration: basicCreateTime } = await this.timer.measure('create-basic', async () => {
    ...
    const { duration: advancedCreateTime } = await this.timer.measure('create-advanced', async () => {
      await this.chromaDB.createCollection(advancedCollectionName, advancedMetadata);
    });
    ...
    for (const collection of specializedCollections) {
      const { duration } = await this.timer.measure(`create-${collection.name}`, async () => {
        await this.chromaDB.createCollection(collection.name, collection.metadata);
      });
  ...
  private async demonstrateCollectionConfiguration(): Promise<void> {
    ...
    this.logger.logResults('Updated configuration', {
      ...
      newFeatures: Object.keys(updatedConfig.features).filter(
        key => !(key in initialConfig.features) || 
        updatedConfig.features[key as keyof typeof updatedConfig.features] !== initialConfig.features[key as keyof typeof initialConfig.features]
      )
    });
  }
  ...
  private async demonstrateCollectionMetadata(): Promise<void> {
    ...
    const richMetadata = {
      ...
      operational: {
        ...
        monitoring: {
          ...
          dashboardUrl: 'https://monitoring.company.com/collections/metadata-demo'
        }
      },
    ...
    const { duration: metadataCreateTime } = await this.timer.measure('create-with-metadata', async () => {
      await this.chromaDB.createCollection(metadataCollectionName, richMetadata);
    });
    ...
    this.logger.logResults(
      ...
      {
        name: metadataCollectionName,
        metadataCategories: Object.keys(richMetadata).filter(key => typeof richMetadata[key as keyof typeof richMetadata] === 'object').length,
    ...
    const metadataValidation = {
      hasRequiredFields: !!(richMetadata.name && richMetadata.description && richMetadata.version),
      hasOwnership: !!(richMetadata.owner && richMetadata.maintainers),
      hasTechnicalSpecs: !!(richMetadata.technical && richMetadata.technical.embeddingModel),
      hasGovernance: !!(richMetadata.governance && richMetadata.governance.dataClassification)
    };
    ...
    try {
      ...
      this.logger.logWarning('Could not retrieve server info from adminService');
    }
  }
  ...
  private async demonstrateCollectionInspection(): Promise<void> {
    ...
    try {
      ...
      const { result: collections, duration: listTime } = await this.timer.measure('list-collections', async () => {
      ...
      this.logger.logResults(
        ...
        { 
          ...
          exampleCollections: collections.filter((name: string) => name.includes('example')).length
        },
        listTime
      );

      // Inspect individual collections
      const exampleCollections = collections.filter((name: string) => name.includes('example'));
      const collectionStats: CollectionStats[] = [];

      for (const collectionName of exampleCollections.slice(0, 3)) { // Limit to first 3 for demo
        try {
          const { result: documents, duration: inspectTime } = await this.timer.measure(
            `inspect-${collectionName}`, 
            async () => {
              return await this.chromaDB.getDocuments(collectionName, { limit: 1 });
            }
          );
          ...
          this.logger.logWarning(`Could not inspect collection ${collectionName}: ${(error as Error).message}`);
        }
      }
      ...
      this.logger.logResults('Collection inspection summary', {
        ...
        totalDocuments: collectionStats.reduce((sum, stats) => sum + stats.documentCount, 0),
        activeCollections: collectionStats.filter(stats => stats.documentCount > 0).length
      });
  ...
  private async demonstrateCollectionMaintenance(): Promise<void> {
    ...
    // Health check
    const { result: healthCheck, duration: healthTime } = await this.timer.measure('health-check', async () => {
      const docs = await this.chromaDB.getDocuments(maintenanceCollectionName);
      return {
    ...
    const { result: perfCheck, duration: perfTime } = await this.timer.measure('performance-check', async () => {
    ...
    this.logger.logResults(
      'Performance check',
      {
        ...
        performanceGrade: perfTime < 100 ? 'excellent' : perfTime < 300 ? 'good' : 'needs-attention'
      },
      perfTime
    );
    ...
    const finalCount = await this.chromaDB.getDocuments(maintenanceCollectionName);
    this.logger.logResults('Maintenance cleanup completed', {
      documentsRemaining: finalCount.ids?.[0]?.length || 0,
      documentsRemoved: testDocuments.length - (finalCount.ids?.[0]?.length || 0)
    });
  }
  ...
  private async cleanupCollections(): Promise<void> {
    ...
    try {
      ...
      const exampleCollections = collections.filter((name: string) => 
        name.includes('example') || name.includes('test') || name.includes('demo')
      );
      ...
      for (const collectionName of exampleCollections) {
        try {
          ...
          this.logger.logWarning(`Could not clean collection ${collectionName}: ${(error as Error).message}`);
        }
      }
   ...
   */
  private countNestedFields(obj: any, depth: number = 0): number {
    let count = 0;
    for (const key in obj) {
      if (obj.hasOwnProperty(key)) {
        count++;
        if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key]) && depth < 3) {
  ...
  async getCollectionsSummary(): Promise<any> {
    try {
      ...
      const summary = {
        ...
        exampleCollections: collections.filter((name: string) => name.includes('example')).length,
        testCollections: collections.filter((name: string) => name.includes('test')).length,
