...
export class ChromaMetricsService {
  ...
  getAggregatedMetrics(): AggregatedMetrics[] {
    ...
    return operationNames.map(operationName => {
      ...
      const aggregated = this.calculateAggregatedMetrics(operationName, operationMetrics);
   ...
   */
  getAggregatedMetricsForOperation(operationName: string): AggregatedMetrics | null {
    // Check cache first
    if (this.aggregationCache.has(operationName)) {
      return this.aggregationCache.get(operationName)!;
    }
  ...
  getPerformanceSummary(timeWindowMinutes = 60): PerformanceSummary {
    ...
    const timeSpanMinutes = Math.max(1, (Date.now() - recentMetrics[0].timestamp.getTime()) / 60000);
    ...
    const aggregatedMetrics = this.getAggregatedMetrics()
      .filter(am => recentMetrics.some(rm => rm.operationName === am.operationName));
    ...
    const mostErrorProne = aggregatedMetrics
      ...
      .sort((a, b) => (b.errorCount / b.totalOperations) - (a.errorCount / a.totalOperations))
      .slice(0, 5)
      .map(am => am.operationName);
  ...
  isOperationUnderperforming(operationName: string): boolean {
    ...
    const highErrorRate = (aggregated.errorCount / aggregated.totalOperations) > 0.1; // >10% error rate
    const slowExecution = aggregated.averageExecutionTime > 10000; // >10 seconds average
    const lowCacheHitRate = aggregated.cacheHitRate < 0.1 && aggregated.totalOperations > 50; // <10% cache hit with enough operations
  ...
  getHealthStatus(): {
    ...
    if (summary.overallSuccessRate < 0.95) { // <95% success rate
      issues.push(`Low success rate: ${(summary.overallSuccessRate * 100).toFixed(1)}%`);
    }

    if (summary.averageExecutionTime > 5000) { // >5 seconds average
      issues.push(`High average execution time: ${summary.averageExecutionTime.toFixed(0)}ms`);
    }
    ...
    if (underperformingOps.length > 0) {
      issues.push(`Underperforming operations: ${underperformingOps.join(', ')}`);
    }
    ...
    if (issues.length === 0) {
      status = 'healthy';
    } else if (issues.length <= 2 && summary.overallSuccessRate > 0.90) {
      status = 'degraded';
    } else {
      status = 'unhealthy';
    }
  ...
  private calculateAggregatedMetrics(
    ...
    const lastOperation = new Date(Math.max(...operationMetrics.map(m => m.timestamp.getTime())));
    
    // Calculate operations per minute based on time span
    const timeSpan = lastOperation.getTime() - operationMetrics[0].timestamp.getTime();
