  ...
  try {
    ...
    if (output) {
      ...
      if (errorCount > 0 || warningCount > 0) {
        console.log(
          `${colors.yellow}  ⚠️  ${errorCount} errors, ${warningCount} warnings remain${colors.reset}`
        );
      }
    } else {
      console.log(
        `${colors.green}  ✅ All fixable issues resolved${colors.reset}`
      );
    }
    ...
    if (errorCount > 0 || warningCount > 0) {
      console.log(
        `${colors.yellow}  ⚠️  ${errorCount} errors, ${warningCount} warnings (some fixed)${colors.reset}`
      );
    } else {
...
async function main() {
  ...
  console.log(
    `${colors.bright}${colors.cyan}🚀 ESLint Auto-Fix Tool${colors.reset}`
  );
  ...
  if (specificPath) {
    ...
    if (existsSync(targetPath)) {
      ...
      console.log(
        `${colors.blue}Targeting specific path:${colors.reset} ${specificPath}\n`
      );
    } else {
      console.error(
        `${colors.red}❌ Path not found:${colors.reset} ${specificPath}`
      );
      process.exit(1);
    }
  } else {
    ...
    console.log(
      `${colors.blue}🔍 Scanning for ESLint configurations...${colors.reset}`
    );
    dirsToFix = findEslintConfigs(rootDir);
    console.log(
      `${colors.green}✅ Found ${dirsToFix.length} projects${colors.reset}\n`
    );
  }
  ...
  console.log(
    `${colors.green}✅ Successfully processed:${colors.reset} ${successful} projects`
  );
  if (withErrors > 0) {
    console.log(
      `${colors.yellow}⚠️  Projects with remaining issues:${colors.reset} ${withErrors}`
    );
  }
  ...
  if (isDryRun) {
    ...
    console.log(
      `   ${colors.cyan}node tools/scripts/eslint-fix-all.mjs${colors.reset}`
    );
  } else {
  ...
  console.log(
    '  • no-unnecessary-type-assertion (removes redundant assertions)'
  );
