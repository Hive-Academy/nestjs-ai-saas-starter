...
export class LangGraphCheckpointProvider {
  ...
  async create(config: CheckpointConfig): Promise<BaseCheckpointSaver> {
    ...
    switch (config.storage) {
      ...
      case 'database':
        this.checkpointer = await this.createSqliteCheckpointer(
          config.storageConfig
        );
        break;

      case 'redis':
        this.checkpointer = await this.createRedisCheckpointer(
          config.storageConfig
        );
        break;

      case 'postgres':
        this.checkpointer = await this.createPostgresCheckpointer(
          config.storageConfig || {}
        );
        break;
      ...
      case 'custom':
        if (config.saver) {
          ...
          throw new Error(
            'Custom storage type requires a saver implementation'
          );
        }
        break;
    ...
    this.logger.log(
      `Created official LangGraph ${config.storage} checkpoint saver`
    );
   ...
   */
  private async createSqliteCheckpointer(
    ...
    try {
      ...
      this.logger.log(
        `Official LangGraph SQLite checkpoint saver initialized at: ${dbPath}`
      );
      return saver;
    } catch (error) {
   ...
   */
  private async createRedisCheckpointer(
    config?: Record<string, any>
  ): Promise<BaseCheckpointSaver> {
    ...
    try {
      ...
      try {
        ...
        const redisModule = await eval(
          `import('@langchain/langgraph-checkpoint-redis')`
        );
        RedisSaver = redisModule.RedisSaver;
      } catch (importError) {
        throw new Error(
          'Official LangGraph Redis checkpoint package not installed. Install @langchain/langgraph-checkpoint-redis to use Redis checkpointing.'
        );
      }
      ...
      this.logger.log(
        'Official LangGraph Redis checkpoint saver created successfully'
      );
      return saver;
    } catch (error) {
      this.logger.warn(
        'Official LangGraph Redis checkpoint package not available, falling back to SQLite'
      );
      return this.createMemoryCheckpointer();
    }
  }
   ...
   */
  private async createPostgresCheckpointer(
    config: Record<string, any>
  ): Promise<BaseCheckpointSaver> {
    try {
      // Dynamic import to avoid hard dependency
      ...
      try {
        // Use eval to prevent static analysis from failing at build time
        const pgModule = await eval(
          `import('@langchain/langgraph-checkpoint-postgres')`
        );
        PostgresSaver = pgModule.PostgresSaver;
      } catch (importError) {
        throw new Error(
          'Official LangGraph Postgres checkpoint package not installed. Install @langchain/langgraph-checkpoint-postgres to use Postgres checkpointing.'
        );
      }
      ...
      this.logger.log(
        'Official LangGraph PostgreSQL checkpoint saver created successfully'
      );
      return saver;
    } catch (error) {
      this.logger.warn(
        'Official LangGraph PostgreSQL checkpoint package not available, falling back to SQLite'
      );
      return this.createMemoryCheckpointer();
    }
  }
