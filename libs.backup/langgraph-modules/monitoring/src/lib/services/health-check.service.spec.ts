...
describe('HealthCheckService', () => {
  ...
  describe('Health Check Registration (AC4 Verification)', () => {
    it('should register health checks successfully', async () => {
      const healthCheck: HealthCheckFunction = jest
        .fn()
    ...
    it('should prevent duplicate registrations', async () => {
      const healthCheck1: HealthCheckFunction = jest
        .fn()
        .mockResolvedValue(true);
      const healthCheck2: HealthCheckFunction = jest
        .fn()
        .mockResolvedValue(false);

      await service.register('duplicate-test', healthCheck1);
      await expect(
        service.register('duplicate-test', healthCheck2)
      ).rejects.toThrow();
    });
  ...
  describe('Individual Health Check Execution', () => {
    ...
    it('should handle failing health checks', async () => {
      const failingCheck: HealthCheckFunction = jest
        .fn()
        .mockResolvedValue(false);
    ...
    it('should handle health check exceptions', async () => {
      const errorCheck: HealthCheckFunction = jest
        .fn()
    ...
    it('should enforce timeout protection (5s max)', async () => {
      const slowCheck: HealthCheckFunction = jest
        .fn()
        .mockImplementation(
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should gracefully handle health check function exceptions', async () => {
      const buggyCheck: HealthCheckFunction = jest
        .fn()
        .mockImplementation(() => {
  ...
  describe('Integration with Monitoring Ecosystem', () => {
    it('should export health status in standard formats', async () => {
      ...
      expect(prometheusFormat).toContain(
        'health_check_status{service="export-test"} 1'
      );
      expect(prometheusFormat).toContain(
        'health_check_status{service="failing-test"} 0'
      );
