...
export class SupervisorCoordinationWorkflow extends DeclarativeWorkflowBase<SupervisorAgentState> {
  ...
  async supervisorNode(
    state: SupervisorAgentState
  ): Promise<Command<SupervisorAgentState>> {
  ...
  async researcherNode(
    state: SupervisorAgentState
  ): Promise<Partial<SupervisorAgentState>> {
  ...
  @RequiresApproval({
    ...
    riskAssessment: {
      ...
      evaluator: (state: SupervisorAgentState) => ({
        ...
        score:
          (state.confidence || 0.5) * (state.complexity === 'high' ? 0.7 : 0.9),
      }),
    },
