...
@Component({
  ...
  template: `
    <div class="w-screen h-screen bg-gradient-to-br from-[#0f0f23] via-[#1a1a3e] to-[#2d2d5f] text-white overflow-hidden relative">

      <!-- Loading Screen -->
      @if (isLoading()) {
        <div class="absolute inset-0 z-[9999] flex flex-col items-center justify-center bg-gradient-to-br from-[#0f0f23] via-[#1a1a3e] to-[#2d2d5f]">

          <!-- AI Logo Animation -->
          <div class="relative mb-8">
            <div class="w-20 h-20 border-4 border-purple-500/30 rounded-full animate-spin">
              <div class="absolute top-1 left-1 w-4 h-4 bg-purple-500 rounded-full animate-pulse"></div>
              <div class="absolute top-1 right-1 w-4 h-4 bg-pink-500 rounded-full animate-pulse" style="animation-delay: 0.5s"></div>
              <div class="absolute bottom-1 left-1 w-4 h-4 bg-sky-500 rounded-full animate-pulse" style="animation-delay: 1s"></div>
              <div class="absolute bottom-1 right-1 w-4 h-4 bg-green-500 rounded-full animate-pulse" style="animation-delay: 1.5s"></div>
            </div>
            <div class="absolute inset-0 w-20 h-20 border-2 border-white/10 rounded-full animate-ping"></div>
          </div>

          <!-- Loading Text -->
          <div class="text-center mb-6">
            <h2 class="text-2xl font-bold bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent mb-2">
              Enterprise AI SaaS Starter
            </h2>
            <p class="text-white/70 text-sm">Initializing intelligent systems...</p>
          </div>
          ...
          <div class="text-center text-white/40 text-xs">
            <div class="flex items-center justify-center gap-4">
              <span [class.text-purple-500]="loadingProgress() >= 20">Vector Intelligence</span>
              <span [class.text-pink-500]="loadingProgress() >= 40">Graph Relations</span>
              <span [class.text-sky-500]="loadingProgress() >= 60">AI Workflows</span>
              <span [class.text-green-500]="loadingProgress() >= 80">3D Visualization</span>
      ...
      <!-- Navigation Dots -->
      @if (!isLoading()) {
        <nav class="fixed right-5 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3 opacity-0 animate-fade-in" style="animation-delay: 0.5s">
      ...
      <!-- Section Progress -->
      @if (!isLoading()) {
      <div class="fixed top-5 left-5 z-50 bg-black/50 backdrop-blur-lg rounded-lg px-4 py-2 text-sm opacity-0 animate-fade-in" style="animation-delay: 0.3s">
        <div class="flex items-center gap-3">
          <span class="text-white/70">{{ currentSectionIndex() + 1 }}/{{ sections().length }}</span>
      ...
      <!-- Demo Controls -->
      @if (!isLoading()) {
      <div class="fixed top-5 right-5 z-50 opacity-0 animate-fade-in" style="animation-delay: 0.7s">
        <button
      ...
      <!-- Kiro Branding -->
      @if (!isLoading()) {
      <div class="fixed bottom-5 left-5 z-50 opacity-0 animate-fade-in" style="animation-delay: 1.2s">
        <div class="flex items-center gap-3 group cursor-pointer">
          ...
          <div class="relative animate-float">
            ...
            <div class="w-10 h-10 flex items-center justify-center text-white/80 text-2xl animate-ghost-glow" style="display:none">
              👻
            </div>
            <!-- Enhanced Glow Effect -->
            <div class="absolute inset-0 w-10 h-10 bg-purple-500/40 rounded-full blur-xl animate-pulse opacity-60 group-hover:opacity-100 transition-opacity duration-500"></div>
            <div class="absolute inset-0 w-10 h-10 bg-gradient-radial from-purple-400/20 via-pink-400/10 to-transparent rounded-full animate-pulse"></div>
          </div>

          <!-- Glowing Text -->
          <div class="text-base font-medium text-white/70 group-hover:text-white transition-all duration-500">
            <span class="bg-gradient-to-r from-purple-300 via-pink-300 to-sky-300 bg-clip-text text-transparent animate-text-glow drop-shadow-lg">
              Built with Kiro
            </span>
            <!-- Text background glow -->
            <div class="absolute inset-0 bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-sky-500/10 rounded-lg blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </div>
        </div>
      </div>
      }
      ...
      <!-- Keyboard Hint -->
      @if (!isLoading()) {
      <div class="fixed bottom-5 right-5 z-50 text-white/50 text-xs opacity-0 animate-fade-in" style="animation-delay: 1s">
  ...
  styles: [`
    ...
    @keyframes ghost-glow {
      0%, 100% {
        filter: brightness(0) invert(1) drop-shadow(0 0 12px rgba(168, 85, 247, 0.6)) drop-shadow(0 0 25px rgba(168, 85, 247, 0.3));
      }
      25% {
        filter: brightness(0) invert(1) drop-shadow(0 0 15px rgba(236, 72, 153, 0.7)) drop-shadow(0 0 30px rgba(236, 72, 153, 0.4));
      }
      50% {
        filter: brightness(0) invert(1) drop-shadow(0 0 18px rgba(14, 165, 233, 0.8)) drop-shadow(0 0 35px rgba(14, 165, 233, 0.5));
      }
      75% {
        filter: brightness(0) invert(1) drop-shadow(0 0 15px rgba(168, 85, 247, 0.7)) drop-shadow(0 0 30px rgba(168, 85, 247, 0.4));
      }
    }
...
export class LandingPageShellComponent implements OnInit, OnDestroy {
  ...
  readonly sections = signal<SectionRoute[]>([
    ...
    { path: '/landing/ecosystem', title: 'Ecosystem', index: 3, id: 'ecosystem' },
    { path: '/landing/architecture', title: 'Architecture', index: 4, id: 'architecture' },
  ]);
  ...
  getNavigationButtonClasses(sectionIndex: number): string {
    ...
    const baseClasses = 'group relative w-4 h-4 border-2 rounded-full transition-all duration-300 hover:scale-125 focus:outline-none focus:ring-2 focus:ring-white/50';
