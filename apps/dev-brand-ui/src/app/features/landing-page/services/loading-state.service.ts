...
export class LoadingStateService {
  ...
  private readonly _loadingStages = signal<LoadingStage[]>([
    { name: 'Initializing', description: 'Setting up the platform', weight: 10 },
    { name: 'Loading 3D Engine', description: 'Preparing Three.js components', weight: 20 },
    { name: 'Loading Hero Section', description: 'Agent constellation setup', weight: 15 },
    { name: 'Loading Platform Pillars', description: '3D pillar visualization', weight: 15 },
    { name: 'Loading Demo Theater', description: 'Interactive demo player', weight: 15 },
    { name: 'Loading Ecosystem Explorer', description: 'Library 3D grid', weight: 15 },
    { name: 'Loading Architecture Diagram', description: 'Layered architecture view', weight: 10 }
  ]);
  ...
  readonly loadingProgress = computed(() => this._globalLoadingState().progress);
  ...
  startLoading(): void {
    ...
    const sections = ['hero', 'platform-pillars', 'demo-theater', 'ecosystem-explorer', 'architecture-diagram'];
  ...
  updateStage(stageName: string): void {
    ...
    const completedWeight = stages.slice(0, stageIndex).reduce((sum, stage) => sum + stage.weight, 0);
