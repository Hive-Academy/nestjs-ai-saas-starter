...
@Component({
  ...
  template: `
  <div class="w-full min-h-screen bg-gradient-to-br from-[#0f0f23] via-[#1a1a3e] to-[#2d2d5f] text-white opacity-0 transition-opacity duration-700 ease-in-out relative" [class.opacity-100]="isLoaded()" #landingContainer>
      ...
      <div class="relative">
        ...
        <div class="absolute top-16 right-0 bg-black/90 border border-white/20 rounded-xl backdrop-blur-lg shadow-2xl overflow-hidden transition-all duration-300 w-72" [class.hidden]="!showDropdownControls()">
          <div class="p-4 space-y-3">
            <div class="text-center text-xs text-white/70 border-b border-white/10 pb-2 mb-3">Page Controls</div>

            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <span class="text-sm text-white/80">Smooth Scroll</span>
                <button class="px-3 py-1 bg-purple-600/30 hover:bg-purple-600/50 rounded text-xs" (click)="toggleSmoothScroll()">Toggle</button>
              </div>

              <div class="flex items-center justify-between">
                <span class="text-sm text-white/80">Navigation Dots</span>
                <button class="px-3 py-1 bg-blue-600/30 hover:bg-blue-600/50 rounded text-xs" (click)="toggleNavigationDots()">Toggle</button>
              </div>
            </div>
            ...
            <div class="border-t border-white/10 pt-3">
              ...
              <div class="flex items-center justify-between mt-2 text-xs">
                <button class="px-2 py-1 bg-sky-600/30 hover:bg-sky-600/50 rounded text-xs" (click)="scrollToTop()">Scroll to Top</button>
                <button class="px-2 py-1 bg-green-600/30 hover:bg-green-600/50 rounded text-xs" (click)="exportSectionInfo()">Export</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <nav class="fixed right-5 top-1/2 -translate-y-1/2 z-[1000] opacity-90 transition-all duration-300 select-none" [class.hidden]="!showNavigationDots()">
      <ul class="flex flex-col gap-4">
        <li *ngFor="let section of sections(); let i = index" 
            class="relative group cursor-pointer"
            [title]="section"
            (click)="scrollToSection(section)">
          <button class="relative w-4 h-4 border-2 border-white/50 rounded-full bg-transparent cursor-pointer transition-all duration-300 hover:border-white hover:scale-125 focus:outline-none focus:ring-2 focus:ring-white/50">
            <span class="absolute inset-0.5 bg-white/30 rounded-full transition-all duration-300"></span>
            <span class="absolute left-6 top-1/2 -translate-y-1/2 whitespace-nowrap bg-black/80 text-white text-xs px-2 py-1 rounded opacity-0 pointer-events-none transition-all duration-300 group-hover:opacity-100">
    ...
    <main class="w-full">
      ...
      <div id="feature-navigation" class="section-container">
        <section class="h-screen w-full flex flex-col items-center justify-center box-border text-center bg-black/30 relative">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-auto-fit gap-6 max-w-6xl mx-auto" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr))">
            <a href="/spatial-interface" class="block p-6 bg-white/5 border border-white/10 rounded-xl no-underline text-white transition-all duration-300 backdrop-blur-lg hover:bg-white/10 hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/30">
            ...
            <a href="/workflow-canvas" class="block p-6 bg-white/5 border border-white/10 rounded-xl no-underline text-white transition-all duration-300 backdrop-blur-lg hover:bg-white/10 hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/30">
              <h4 class="m-0 mb-2 text-xl text-purple-500">Workflow Canvas</h4>
              <p class="m-0 text-white/70 text-sm">Visual workflow designer</p>
            </a>
            <a href="/memory-constellation" class="block p-6 bg-white/5 border border-white/10 rounded-xl no-underline text-white transition-all duration-300 backdrop-blur-lg hover:bg-white/10 hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/30">
            ...
            <a href="/chat-interface" class="block p-6 bg-white/5 border border-white/10 rounded-xl no-underline text-white transition-all duration-300 backdrop-blur-lg hover:bg-white/10 hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/30">
            ...
            <a href="/content-forge" class="block p-6 bg-white/5 border border-white/10 rounded-xl no-underline text-white transition-all duration-300 backdrop-blur-lg hover:bg-white/10 hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/30">
...
export class LandingPageComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private setupSmoothScrolling(): void {
    if (typeof window !== 'undefined') {
      ...
      document.documentElement.style.scrollBehavior = this.smoothScrollEnabled() ? 'smooth' : 'auto';
    }
  }
  ...
  scrollToSection(sectionId: string): void {
    ...
    if (element) {
      element.scrollIntoView({ behavior: this.smoothScrollEnabled() ? 'smooth' : 'auto' });
    }
  }

  scrollToTop(): void {
    window.scrollTo({ top: 0, behavior: this.smoothScrollEnabled() ? 'smooth' : 'auto' });
  }
